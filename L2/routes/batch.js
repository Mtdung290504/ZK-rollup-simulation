...
router.post('/batch/submit-proof', async (req, res) => {
	const { proof, publicSignals, oldStateRoot, newStateRoot, daRoot, num_deposits, transactions } = req.body;
	...
	try {
		...
		const archiveRes = await fetch('http://localhost:4000/archive/blobs', {
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify({ batch_id: l1Data.batch_id, transactions }),
		});
		if (!archiveRes.ok) {
